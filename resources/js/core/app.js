      ...
      if (value != '') {
        ...
        var $startList = '',
          ...
          $pageList =
            ...
            `<a href="${assetPath}app/inquiries/list">` +
            '<h6 class="section-label mt-75 mb-0">Enquiries</h6>' +
            '</a>' +
            '</li>',
          $activeItemClass = '',
          $bookmarkIcon = '',
        ...
        $.ajax({
          url: `${assetPath}app/inquiries/search`, // Replace with the actual API URL
          type: 'GET',
          dataType: 'json',
          data: { query: value  }, // Pass the query parameter
          success: function(data) {
            console.log(data.listItems,'data')
            if (data.listItems.length < 1) {
              $startList =
              '<li class="auto-suggestion"' +
              '<div class="d-flex align-items-center justify-content-between w-100" href="#">' +
